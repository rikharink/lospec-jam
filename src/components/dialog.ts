...
export class Dialog extends ContentView {
    constructor({
        title = '',
        margin = 20,
        ...
        dialog.drawRoundedRect(margin, margin, width - margin * 2, height - margin * 2, borderRadius)
        dialog.endFill();
        if (title) {
        ...
        if (cancel) {
            let cancelBtn = new Button({ text: cancelLabel, activeColor: Palette[7] });
